...
def get_debater_configs():
    debater_dir = Path("./core/config/playground/debaters")
    consultant_dir = Path("./core/config/playground/consultants")
...
async def create_debate(payload: CreateDebatePayload):
    ...
    if payload.previous_debate_id:
        ...
        transcript = create_live_debate_transcript(question, payload.debate_type)
    ...
    debate_fields = CreateDebateInput(
        ...
        experiment_id=None,
        user_id=999,  # lazy hack -> 999 will be the "public playground" user
    )
